...
import type { PurchaseOrdersListItem, PurchaseOrdersListQuery } from "@/lib/purchase-orders/types";
...
export function usePurchaseOrdersList(options?: {
  limit?: number;
  initialFilters?: Omit<PurchaseOrdersListQuery, "limit" | "cursor">;
}) {
  const tc = useTranslations("Common");
  const limit = options?.limit ?? 10;

  const [filters, setFilters] = useState<Omit<PurchaseOrdersListQuery, "limit" | "cursor">>(
    ...
    async (cursor: string | null, mode: "replace" | "append") => {
      ...
      try {
        ...
        setState((s) => {
          ...
          const deduped: PurchaseOrdersListItem[] = [];
