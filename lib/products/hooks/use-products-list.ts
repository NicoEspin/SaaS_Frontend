...
export function useProductsList(options?: {
  limit?: number;
  initialFilters?: Omit<ProductsListQuery, "limit" | "cursor">;
}) {
  const tc = useTranslations("Common");
  const limit = options?.limit ?? 10;

  const [filters, setFilters] = useState<Omit<ProductsListQuery, "limit" | "cursor">>(
  ...
  const fetchPage = useCallback(
    async (cursor: string | null, mode: "replace" | "append") => {
      ...
      try {
        ...
        setState((s) => {
          const nextItems =
            mode === "replace" ? res.items : [...s.items, ...res.items];
