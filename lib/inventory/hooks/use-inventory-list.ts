...
export function useInventoryList(branchId: string | null, options?: { limit?: number }) {
  ...
  const fetchPage = useCallback(
    async (cursor: string | null, mode: "replace" | "append") => {
      ...
      try {
        const res = await inventoryApi.getInventoryByBranch(
          branchId,
          {
            limit,
        ...
        setState((s) => {
          ...
          const deduped: BranchInventoryItem[] = [];
