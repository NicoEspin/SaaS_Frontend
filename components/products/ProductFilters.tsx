...
function isIsActiveValue(value: string): value is ProductsUiFilters["isActive"] {
...
export function ProductFilters({ value, onChange, onApply, onReset, loading }: Props) {
  ...
  }, [value.categoryId, value.categoryName, value.code, value.isActive, value.name, value.q]);

  const hasAdvancedValues = Boolean(
    value.code.trim() || value.name.trim() || value.categoryId.trim() || value.categoryName.trim()
  );
    ...
    <Card>
      <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-3 space-y-0">
        ...
        <div className="flex w-full items-center justify-end gap-2 sm:w-auto">
          ...
          <Button type="submit" form="products_filters" disabled={loading}>
